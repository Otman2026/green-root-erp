...
import { ArrowRight, Bug, Sprout, Beaker, Package, Loader2 } from "lucide-react";
import { toast } from "sonner";

export const Route = createFileRoute("/_authenticated/agri/pests/$id")({ component: PestDetail });
...
function PestDetail() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      const p = await supabase.from("agri_pests").select("*").eq("id", id).maybeSingle();
      if (p.error) { toast.error(p.error.message); setLoading(false); return; }
      setPest(p.data);

      const [links, ts] = await Promise.all([
        supabase.from("agri_plant_pests").select("plant_id").eq("pest_id", id),
        supabase.from("agri_treatments").select("*").eq("target_type", "pest").eq("target_id", id),
      ]);
      ...
      if (p.data?.name_ar) {
        const { data } = await supabase.from("agri_pesticides").select("*").ilike("target_pests", `%${p.data.name_ar}%`);
        setPesticides(data ?? []);
        const productIds = (data ?? []).map((x: any) => x.product_id).filter(Boolean);
  ...
  if (!pest) return <div className="p-6">لم يتم العثور على الآفة</div>;

  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center gap-3">
        <Button size="sm" variant="ghost" onClick={() => nav({ to: "/agri/pests" })}><ArrowRight className="w-4 h-4" /></Button>
        <Bug className="w-6 h-6 text-orange-600" />
        <div>
          <h1 className="text-2xl font-bold">{pest.name_ar}</h1>
          <p className="text-sm text-muted-foreground italic">{pest.scientific_name} {pest.name_fr && `· ${pest.name_fr}`}</p>
        </div>
        {pest.severity && <Badge variant="destructive" className="ms-auto">شدة: {pest.severity}</Badge>}
      </div>

      <div className="grid gap-4 md:grid-cols-2">
        <Card className="p-4 space-y-2">
          <Row label="النوع" value={pest.type} />
          <Row label="الوصف" value={pest.description} />
          <Row label="دورة الحياة" value={pest.life_cycle} />
          <Row label="الأضرار" value={pest.damage} />
        </Card>
        <Card className="p-4">
          <h3 className="font-semibold mb-2">مراجع</h3>
          <p className="text-sm whitespace-pre-line">{pest.refs || "—"}</p>
        </Card>
      </div>

      {plants.length > 0 && (
        <Card className="p-4">
          <h3 className="font-semibold mb-2 flex items-center gap-2"><Sprout className="w-4 h-4 text-green-600" />المحاصيل المستهدفة</h3>
      ...
      {treatments.length > 0 && (
        <Card className="p-4">
          <h3 className="font-semibold mb-2">🔬 بروتوكولات المكافحة ({treatments.length})</h3>
